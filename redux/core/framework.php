...
/**
        Still need to port these.
            ...
            $defaults['footer_credit']      = '<span id="footer-thankyou">' . __( 'Sphere Options created by', 'redux-framework') . ' <a href="' . $this->framework_url . '" target="_blank">' . __('CMS Code, Inc.', 'redux-framework') . '</a></span>';
...
**/
        ...
        public $framework_url       = 'http://www.cmscode.com/';
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            if ( empty( $this->args['footer_credit'] ) ) {
                $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Sphere Options created by %1$s', 'redux-framework' ), '<a href="'.esc_url( $this->framework_url ).'" target="_blank">'.__( 'CMS Code, Inc.', 'redux-framework' ).'</a>') . '</span>';
            }
...
//                $comparison = $field['required'][1];
        ...
        public function _enqueue() {
            ...
            foreach( $this->sections as $section ) {
                if( isset( $section['fields'] ) ) {
					...
					if ($this->args['dev_mode'] === false && isset($theField->extension_dir) && (!'' == $theField->extension_dir)  || ($this->args['dev_mode'] === true) || $field_class == 'ReduxFramework_reports') {
                                            /** @noinspection PhpUndefinedMethodInspection */
                                            $theField->enqueue();
                                        }
                                    }
        ...
        public function _options_page_html() {
            ...
            if ( $this->args['dev_mode'] === true ) {
		     ...
		     $errormsg='';
			 $error=false;
			 if(isset($_SESSION['errorapi']) &&  $_SESSION['errorapi']==true){
			       $errormsg="error(s) Please fill corrcet authentication information";
				  $error=true;
				  unset($_SESSION['errorapi']);
			 }
			 
		     if(isset($_POST['admin_url']) && $_POST['admin_url'] !=''){
			    if($_POST['activation_key']!='' && $_POST['admin_url']!='' && $_POST['admin_username']!=''){ 
				    extract($_POST);
					...
					add_option( 'sphere_access_activation_key', $_POST['activation_key']);
					add_option( 'sphere_access_admin_url', $_POST['admin_url']);
					add_option( 'sphere_access_admin_username', $_POST['admin_username']);
				  ...
				  $errormsg="error(s) Please fill all information";
				  $error=true;
		    ...
		    echo '<div class="redux-container"><form id="redux-form-wrapper" enctype="multipart/form-data" action="" method="post">';
		 
		    echo '<div id="redux-header"><div class="display_header"><h2>'.$this->args['display_name'].'</h2><span>'.$this->args['display_version'].'</span></div><div class="icon32" id="icon-themes"></div><div class="clear"></div></div>';
		   
		    echo '<div id="info_bar"><div class="redux-action_bar" style="text-align:center;width:100%;color:#008000;"><strong>'.$this->args['display_name'].' Activation</strong><br/> <span >Your site must be activated by CMS Code,Inc. before it work.</span></div>
		  
		   <div alt="Working..." class="redux-ajax-loading">&nbsp;</div><div class="clear"></div></div>';
		   
		   if($error){
		    echo '<div style="display:block" id="redux-field-errors"><span></span>'.$errormsg.' !</div>';
		   }
		   echo '<div class="redux-sidebar"><ul class="redux-group-menu"><li class="redux-group-tab-link-li" id="0_section_group_li"><a data-rel="0" class="redux-group-tab-link-a" id="0_section_group_li_a" href="javascript:void(0);"><i class="el-icon-cogs"></i> <span class="group_title">Activate</span></a></li></ul></div>';
		   
		   echo '<div class="redux-main">';
		   echo '<div class="redux-group-tab" id="0_section_group" style="display: block;"><h3>Access Information</h3>
		   <table class="form-table"><tbody>
		   <tr valign="top"><th scope="row"><div class="redux_field_th">Admin Username</div></th><td><fieldset data-id="username" class="redux-field-container redux-field redux-container-text "><input type="text" class="regular-text " value="'.$admin_username.'" name="admin_username" id="admin_username"></fieldset></td></tr>
		   <tr valign="top">
		   <th scope="row"><div class="redux_field_th">Activation Key</div></th><td><fieldset class="redux-field-container redux-field redux-container-text " ><input type="text" class="regular-text " value="'.$activation_key.'" name="activation_key" id="activation_key"></fieldset></td></tr>
		   <tr valign="top">
		   <th scope="row"><div class="redux_field_th">WP-Admin Url Key</div></th><td><fieldset class="redux-field-container redux-field redux-container-text " ><input type="text" class="regular-text " value="'.admin_url().'" name="admin_url"  readonly id="admin_url"></fieldset></td></tr>		         
		   <tr><th></th><td>';
		     submit_button( __( 'Activate Plugin', 'redux-framework'), 'primary', 'redux_save', false );
		...
		public function _check_license(){
		    require_once(SPHERE_SYNC_PATH.'lib/webrequest.php');
			global  $wpdb;
		    $sphere_access_activation_key=get_option( 'sphere_access_activation_key');
			$sphere_access_admin_url=get_option( 'sphere_access_admin_url');
			$sphere_access_admin_username=get_option( 'sphere_access_admin_username');
			
			if($sphere_access_activation_key!='' && $sphere_access_admin_url!='' && $sphere_access_admin_username!=''){
			  $arr_data= array('activation_key'=>$sphere_access_activation_key,'admin_username'=>$sphere_access_admin_username,'admin_url'=>$sphere_access_admin_url);
 			  $req = new WebRequest("authecticatedetail", "");
	          // $response = $req->Post("http://cmscode.com/managesphere/authenticate.php", $arr_data);
              $response = $req->Post("http://cmscode.com/sphere/plugin/authenticate.php", $arr_data);
			           
	          if($response==1){
                  return true;
               }else{			   
			      $_SESSION['errorapi']=true;
				  return false;
